...
/**
 * g7-core-zh-CN 언어팩 ↔ 한국어 코어 원본 정합성 검사기 (standalone).
 *
 * 용도
 * ----
 * `lang-packs/_bundled/g7-core-zh-CN/` 의 번역 산출물이 한국어 원본(`lang/ko/`,
 * `lang/ko.json`, `lang/partial/ko/`)과 구조적으로 1:1 대응하는지 검사한다.
 * Laravel 부팅도 Composer autoload 도 필요하지 않아 vendor/ 가 없는 환경에서도
 * `php tests/Translations/zh-CN-core-parity-check.php` 로 단독 실행된다.
 *
 * 검사 항목 (요구사항 1~17 대응)
 * ------------------------------
 *  1. 파일 인벤토리    — 원본 대비 파일 1:1 대응 (누락/초과)
 *  2. 키 누락          — 원본에 있고 번역에 없는 dot-path
 *  3. 초과 키          — 번역에만 있는 dot-path
 *  4. 키 순서          — 동일 깊이에서의 키 나열 순서 일치
 *  5. 자료형           — array/string/int/bool/null 일치
 *  6. placeholder      — :attr, {var}, {{var}}, %s/%d 집합 일치
 *  7. HTML 태그        — 태그명 + 속성 집합 일치
 *  8. URL              — http(s):// 토큰 집합 일치
 *  9. 빈 값            — 원본이 빈 문자열이면 번역도 빈 문자열
 * 10. 한글 잔존        — 번역 값에 남은 Hangul (미번역 검출)
 * 11. 일본어 잔존      — 번역 값에 섞인 히라가나/가타카나 (ja 팩 오염 검출)
 * 12. UTF-8 / BOM      — 인코딩 유효성 및 BOM 부재
...
$packRoot = $root.'/lang-packs/_bundled/g7-core-zh-CN';
$locale = 'zh-CN';
 ...
 *
 * 리스트(연속 정수 키) 도 인덱스를 경로에 포함해 구조를 그대로 보존한다.
 *
 * @param  mixed  $node
...
function placeholders(string $s): array
{
    ...
    // Laravel :placeholder.
    // 앞 문자로 걸러내지 않는다 — `v:version` / `:sizeMB` 처럼 영숫자에 붙은 형태도
    // Laravel 의 str_replace 는 실제로 치환하므로 placeholder 다. 대신 `::x` 만
    // 중복 계수되지 않도록 바로 앞의 콜론만 제외한다. `core:update` 같은 명령 문자열도
    // 토큰으로 잡히지만, 명령은 원문 그대로 보존되므로 양쪽이 동일하게 잡혀 무해하고,
    // 오히려 명령을 잘못 번역하면 불일치로 검출된다.
...
{
    $found = [];
    // 종결 문장부호는 URL 의 일부가 아니다 — ASCII 뿐 아니라 중국어 전각 괄호·구두점
    // (）。，、；：「」“”) 도 경계로 취급해야 zh-CN 번역에서 거짓 불일치가 나지 않는다.
...
/**
 * 파일이 BOM 없이 시작하며 UTF-8 로 유효한지 검사한다.
 */
...
function compareTree($ko, $zh, string $label): void
{
    ...
    foreach ($koFlat as $path => $koVal) {
        ...
        // 한글 잔존 (미번역)
        if (preg_match('/[\x{AC00}-\x{D7A3}\x{1100}-\x{11FF}\x{3130}-\x{318F}]/u', $zhVal)) {
            fail('hangul_residue', "$label :: $path — \"".mb_substr($zhVal, 0, 60).'"');
        }

        // 일본어 가나 잔존 (ja 팩 오염)
        if (preg_match('/[\x{3040}-\x{309F}\x{30A0}-\x{30FF}]/u', $zhVal)) {
        ...
        // 표기 스타일 (실패 아님) — CJK 문자 바로 옆의 ASCII 문장부호
...
$koPhp = glob($koDir.'/*.php') ?: [];
$zhPhp = glob($zhDir.'/*.php') ?: [];

$koNames = array_map('basename', $koPhp);
$zhNames = array_map('basename', $zhPhp);
sort($koNames);
sort($zhNames);
...
// ── 2. 프론트엔드 JSON 대조 ───────────────────────────────────────────
// 원본 lang/ko.json 의 $partial 경로는 "partial/ko/x.json", 팩은 "partial/x.json"
// 으로 정규화된다 (ja 팩과 동일 규칙). 경로 값 자체는 번역 대상이 아니므로
...
$jsonPairs = [
    ['ko' => $root.'/lang/ko.json', 'zh' => $packRoot.'/frontend/'.$locale.'.json', 'label' => "frontend/$locale.json"],
    ['ko' => $root.'/lang/partial/ko/errors.json', 'zh' => $packRoot.'/frontend/partial/errors.json', 'label' => 'frontend/partial/errors.json'],
    ['ko' => $root.'/lang/partial/ko/layout_editor.json', 'zh' => $packRoot.'/frontend/partial/layout_editor.json', 'label' => 'frontend/partial/layout_editor.json'],
];

foreach ($jsonPairs as $pair) {
    if (! is_file($pair['ko'])) {
        fail('structure', "원본 누락: {$pair['ko']}");

        continue;
    }
    if (! is_file($pair['zh'])) {
        fail('missing_files', $pair['label']);

        continue;
    }

    checkEncoding($pair['zh'], $pair['label']);

    $koData = json_decode((string) file_get_contents($pair['ko']), true);
    $zhData = json_decode((string) file_get_contents($pair['zh']), true);

    if (json_last_error() !== JSON_ERROR_NONE) {
        fail('json_syntax', $pair['label'].': '.json_last_error_msg());

        continue;
    }
    if (! is_array($koData) || ! is_array($zhData)) {
        fail('json_syntax', $pair['label'].': 최상위가 객체가 아닙니다');

        continue;
    }

    compareTree(normalizePartialPaths($koData, 'ko'), $zhData, $pair['label']);
    $stats['files']++;
    $stats['json_files']++;
}

// ── 3. $partial 경로 실재 확인 ────────────────────────────────────────
$entry = $packRoot.'/frontend/'.$locale.'.json';
if (is_file($entry)) {
    $data = json_decode((string) file_get_contents($entry), true);
...
// ── 4. seed JSON 구문/인코딩 검사 ─────────────────────────────────────
// seed 원본은 config/core.php · 시더에 분산되어 있어 기계적 1:1 대조 대상이
// 아니다. 여기서는 구문·인코딩·잔존 문자만 검사하고, 키 대칭은 ja 팩과
// 대조한다 (동일 코어 원본에서 생성된 산출물이므로 키 집합이 같아야 한다).
$jaSeedDir = $root.'/lang-packs/_bundled/g7-core-ja/seed';
...
foreach ($jaSeeds as $name) {
    ...
    foreach ($zhFlat as $path => $val) {
        ...
        if (preg_match('/[\x{AC00}-\x{D7A3}\x{1100}-\x{11FF}\x{3130}-\x{318F}]/u', $val)) {
            fail('hangul_residue', "seed/$name :: $path — \"".mb_substr($val, 0, 60).'"');
        }
        if (preg_match('/[\x{3040}-\x{309F}\x{30A0}-\x{30FF}]/u', $val)) {
...
echo "=== g7-core-zh-CN parity check ===\n";
echo sprintf(

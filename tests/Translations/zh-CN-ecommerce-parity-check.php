...
/**
 * g7-module-sirsoft-ecommerce-zh-CN 언어팩 ↔ 이커머스 모듈 한국어 원본 정합성 검사기 (standalone).
 *
 * 용도
 * ----
 * `lang-packs/_bundled/g7-module-sirsoft-ecommerce-zh-CN/` 의 번역 산출물이 이커머스 모듈의
 * 한국어 원본(`modules/_bundled/sirsoft-ecommerce/src/lang/ko/`,
 * `modules/_bundled/sirsoft-ecommerce/resources/lang/ko.json`,
 * `.../resources/lang/partial/ko/`)과 구조적으로 1:1 대응하는지 검사한다.
 * Laravel 부팅도 Composer autoload 도 필요하지 않아 vendor/ 가 없는 환경에서도
 * `php tests/Translations/zh-CN-ecommerce-parity-check.php` 로 단독 실행된다.
 *
...
$packRoot = $root.'/lang-packs/_bundled/g7-module-sirsoft-ecommerce-zh-CN';
$jaPackRoot = $root.'/lang-packs/_bundled/g7-module-sirsoft-ecommerce-ja';
$moduleRoot = $root.'/modules/_bundled/sirsoft-ecommerce';
$locale = 'zh-CN';

/**
 * seed 에서 ja 팩에는 없으나 zh-CN 에는 의도적으로 포함한 키.
 *
 * ja 팩(v1.1.3)이 이커머스 모듈의 현재 ko 원본보다 오래되어 아래 항목이 빠져 있다.
 * 이 팩은 ja 재번역이 아니라 ko 원본(module.php / 시더)을 기준으로 만들었으므로
 * 해당 키를 포함한다 — ja 팩은 별도 동기화 대상이며, 이 검사기가 그 차이를
 * "초과 키" 로 잘못 보고하지 않도록 여기 명시한다.
 *
 * - permissions: module.php::getPermissions() 의 dashboard / mileage /
 *   user-currency / user-shipping-country 카테고리 (ja 에 없음)
 * - menus: module.php::getAdminMenus() 의 sirsoft-ecommerce-mileage-transactions
 * - notifications: module.php::getNotificationDefinitions() 의
 *   order_pending_deposit / mileage_expiring_soon / order_delivered
 *
 * @var array<string, array<int, string>> seed 파일명 ⇒ 허용된 추가 dot-path 목록
 */
$intentionalSeedExtras = [
    'menus.json' => [
        'sirsoft-ecommerce-mileage-transactions.name',
    ],
    'permissions.json' => [
        'sirsoft-ecommerce.dashboard.name',
        'sirsoft-ecommerce.dashboard.description',
        'sirsoft-ecommerce.dashboard.view.name',
        'sirsoft-ecommerce.dashboard.view.description',
        'sirsoft-ecommerce.mileage.name',
        'sirsoft-ecommerce.mileage.description',
        'sirsoft-ecommerce.mileage.read.name',
        'sirsoft-ecommerce.mileage.read.description',
        'sirsoft-ecommerce.mileage.manage.name',
        'sirsoft-ecommerce.mileage.manage.description',
        'sirsoft-ecommerce.user-currency.name',
        'sirsoft-ecommerce.user-currency.description',
        'sirsoft-ecommerce.user-currency.manage.name',
        'sirsoft-ecommerce.user-currency.manage.description',
        'sirsoft-ecommerce.user-shipping-country.name',
        'sirsoft-ecommerce.user-shipping-country.description',
        'sirsoft-ecommerce.user-shipping-country.manage.name',
        'sirsoft-ecommerce.user-shipping-country.manage.description',
    ],
    'notifications.json' => [
        'order_pending_deposit.definition.name',
        'order_pending_deposit.definition.description',
        'order_pending_deposit.templates.mail.subject',
        'order_pending_deposit.templates.mail.body',
        'order_pending_deposit.templates.database.subject',
        'order_pending_deposit.templates.database.body',
        'mileage_expiring_soon.definition.name',
        'mileage_expiring_soon.definition.description',
        'mileage_expiring_soon.templates.mail.subject',
        'mileage_expiring_soon.templates.mail.body',
        'mileage_expiring_soon.templates.database.subject',
        'mileage_expiring_soon.templates.database.body',
        'order_delivered.definition.name',
        'order_delivered.definition.description',
        'order_delivered.templates.mail.subject',
        'order_delivered.templates.mail.body',
        'order_delivered.templates.database.subject',
        'order_delivered.templates.database.body',
    ],
];
 ...
 *
 * ja 팩이 만들어진 뒤 ko 원본이 바뀐 자리다. 이 팩의 정합 기준은 ko 원본이므로
 * ja 와의 placeholder·태그 개수 차이를 실패로 보지 않는다. 대신 여기 명시해
 * "왜 다른가" 가 코드에 남도록 한다. 목록에 없는 차이는 여전히 실패다.
 *
 * - order_confirmed.templates.mail.body: ko 원본이 수취인/배송국가/배송지 3행을
 *   추가(module.php::orderConfirmedDefinition())했으나 ja 팩에는 반영되지 않았다.
 *
 * @var array<string, array<int, string>> seed 파일명 ⇒ 값 대조 제외 dot-path 목록
 */
$intentionalSeedValueDrift = [
    'notifications.json' => [
        'order_confirmed.templates.mail.body',
    ],
];
 ...
 *
 * `admin/mileage_deposit_settings.json` 은 ko·en 원본 어디에도 없고 모듈 코드 참조도 0건인,
 * ja 팩에만 남은 사문(死文)이다. 이 팩은 ja 복제가 아니라 ko 원본을 기준으로 만들었으므로
 * 포함하지 않는다 — 아래 목록은 ja↔zh 구조 대조 시 그 차이를 의도된 것으로 표시한다.
 *
 * @var array<int, string> ja 팩 기준 상대경로
 */
$intentionalJaOnlyPartials = [
    'admin/mileage_deposit_settings.json',
];
...
echo "=== g7-module-sirsoft-ecommerce-zh-CN parity check ===\n";
echo sprintf(

...
/**
 * 공식 페이지 모듈 중국어 간체 번들 언어팩(g7-module-sirsoft-page-zh-CN)의 무결성 검증.
 *
 ...
 * 릴리즈까지 통과한다. 이 테스트가 그 사각을 페이지 zh-CN 팩에 대해 닫는다.
 *
 * 검증 대상:
 ...
 *  9. 대상 모듈(sirsoft-page)이 저장소에 실재
 *
 * 번들 미배포 상태에서는 자동 스킵 (CI 안전).
 */
class BundledSimplifiedChinesePagePackTest extends TestCase
{
    /** 검증 대상 패키지 식별자 */
    private const IDENTIFIER = 'g7-module-sirsoft-page-zh-CN';

    /** 대상 모듈 식별자 */
    private const TARGET = 'sirsoft-page';
     ...
     *
     * 키 대칭 검사는 값을 보지 않는다. `:title` 을 빠뜨리거나 `{{count}}` 를
     * `{count}` 로 바꿔 써도 예외가 나지 않고, 화면에 치환되지 않은 원문이나
     * 빈 자리가 그대로 노출된다. 종류뿐 아니라 개수까지 비교한다.
     *
     * @return void
     */
    public function test_placeholders_match_korean_origin(): void
    {
        $this->skipUnlessBundled();
        ...
        $koDir = $this->moduleRoot().'/src/lang/ko';
        $zhDir = $this->packageRoot().'/backend/'.self::LOCALE;
        foreach (array_map('basename', File::glob($koDir.'/*.php') ?: []) as $name) {
            $ko = $this->flattenValues(require $koDir.'/'.$name);
            $zh = $this->flattenValues(require $zhDir.'/'.$name);
            foreach ($ko as $key => $value) {
                $expected = $this->placeholderCounts($value);
                $actual = $this->placeholderCounts($zh[$key] ?? '');
                if ($expected !== $actual) {
                    $mismatches[] = "$name :: $key";
                }
            }
        ...
        $pairs = [
            'ko.json' => [
                $this->moduleRoot().'/resources/lang/ko.json',
                $this->packageRoot().'/frontend/'.self::LOCALE.'.json',
            ],
            'partial/admin.json' => [
                $this->moduleRoot().'/resources/lang/partial/ko/admin.json',
                $this->packageRoot().'/frontend/partial/admin.json',
            ],
        ];
    ...
    /**
     * seed 번역이 모듈이 실제로 선언한 식별자와 대응하는지 검증합니다.
     *
     * seed 는 설치 시 DB 로 들어가므로 식별자가 어긋나면 번역이 조용히 버려진다.
     * 권한 키는 `module.php::getPermissions()` 가, 메뉴 slug 는 `getAdminMenus()` 가
     * 단일 출처다 — 여기서는 그 계약대로 생성되는 식별자를 고정한다.
     *
     * @return void
     */
    public function test_seed_identifiers_match_module_declaration(): void
    {
        $this->skipUnlessBundled();

        $seedDir = $this->packageRoot().'/seed';

        $permissions = json_decode(File::get($seedDir.'/permissions.json'), true);
        $this->assertSame(
            [
                'sirsoft-page',
                'sirsoft-page.pages',
            ...
            'permissions seed 는 module.php::getPermissions() 의 식별자와 순서까지 일치해야 함'
        );
        foreach ($permissions as $key => $entry) {
            $this->assertArrayHasKey('name', $entry, "$key 에 name 이 필요");
            $this->assertArrayHasKey('description', $entry, "$key 에 description 이 필요");
        }

        $menus = json_decode(File::get($seedDir.'/menus.json'), true);
        $this->assertSame(
            ['sirsoft-page'],
            array_keys($menus),
            'menus seed 는 module.php::getAdminMenus() 의 slug 와 일치해야 함'
        );

        $manifest = json_decode(File::get($seedDir.'/manifest.json'), true);
        $this->assertSame(['name', 'description'], array_keys($manifest));

        // getRoles() 가 빈 배열이므로 roles seed 는 존재하면 안 된다.
        $this->assertFalse(
            File::isFile($seedDir.'/roles.json'),
            '페이지 모듈은 역할을 선언하지 않으므로 roles seed 가 있으면 안 된다'
        );
    }

...
`name` 은 그대로 남습니다. 비활성 상태에서 **이후 이커머스 메뉴 재동기화가 실행되면** 그때
원래 모듈 정의 계층이 다시 적용될 수 있지만, 그건 재동기화의 효과이지 비활성화의 효과가
아닙니다. 게다가 비활성화하면 `yutiv:admin-menu` 명령이 등록 해제되어 `--rollback` 을 쓸 수
없게 됩니다.
